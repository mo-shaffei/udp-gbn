...
class Receiver:
    ...
    def receive_file(self, filename: str) -> None:
        ...
        while 1:  # loop until all packets have been received
            ...
            packet, sender_address = self.socket.recvfrom(self.max_seg_size + 6)
            ...
            if packet_id == self.expectedseqnum:  # if packet id matched expectedseqnum, then accept it
                ...
                if trailer_bits.hex() == 'ffff':  # if trailer_bits is 'ffff' then all data is received
                    break
                self.expectedseqnum += 1  # increment expectedseqnum
    ...
    def _send_ack(self, sender_address, file_id):
        ...
        :return:
        """
        packet = self.expectedseqnum.to_bytes(2, sys.byteorder)  # create ack packet and add expectedseqnum to it
        packet += file_id  # add file id to packet
        self.socket.sendto(packet, sender_address)  # send packet
